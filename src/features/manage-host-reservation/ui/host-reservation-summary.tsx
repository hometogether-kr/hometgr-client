      ...
      <HostPanel className="mx-auto w-full max-w-[500px]">
        <dl className="space-y-6">
          ...
          <div>
            ...
            <dd>{usesLiveApi ? "상세 장소 정보 없음" : "서울특별시 마포구 연남동"}</dd>
          </div>
        </dl>
      </HostPanel>
    );
  return (
    <HostPanel>
      ...
      <div className="flex items-center gap-6 border-b border-grayscale-200 pb-8">
        {usesLiveApi ? (
          <div className="flex h-[132px] w-[180px] items-center justify-center rounded-lg bg-grayscale-100 text-grayscale-500 max-sm:h-24 max-sm:w-28">
            매물
          </div>
        ) : (
          <img
        ...
        <div>
          <p className="text-xl text-grayscale-700">
            {usesLiveApi ? "가격 정보 없음" : "월세 1,000/80"}
          </p>
          <h3 className="mt-3 text-2xl font-semibold">
            {usesLiveApi ? `매물 ${reservation.roomId.slice(0, 8)}` : "마포구 연남동 신축 투룸"}
          </h3>
          <p className="mt-6 text-body-1 text-grayscale-500">
            {usesLiveApi ? "위치 정보 없음" : "서울특별시 마포구 연남동"}
          </p>
        </div>
      </div>
      ...
      <p className="rounded-lg bg-grayscale-70 p-6 text-lg">{reservation.studentMessage}</p>
    </HostPanel>
  );
}
