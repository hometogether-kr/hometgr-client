...
> **결정: 전부 화면 기준으로 맞춥니다.** 아래 표의 "화면" 열이 확정 스펙입니다.
...
| #   | 항목           | 갱신된 계약                                                                      | 프론트 반영                                                      |
| --- | -------------- | -------------------------------------------------------------------------------- | ---------------------------------------------------------------- |
| 1   | 자기소개       | `PutMeDto`의 `introduction`이 **optional**로 변경                                | 플레이스홀더 문자열 전송 불필요                                  |
| 2   | 사진 장수      | `mediaIds` **1~20장**, 활성 미디어 합계 최대 20 (요청당 파일은 여전히 10개)      | 화면 6~20장 그대로. 업로드만 10장씩 분할 전송                    |
| 3   | 주소           | step 3이 `buildingType`만 required. 정확 주소 3종 · 대략적 위치 중 택1 (`oneOf`) | 정확 주소를 보낼 땐 3종 모두 요구하도록 검증 강화                |
| 4   | 위치기반 약관  | `ConsentKey`에 `locationBasedServiceTerms` 추가                                  | 동의 키 enum에 반영                                              |
| 5   | 최종 확인 약관 | step 11에 `roomPublication` · `noFraudPledge` 추가 (**true만 허용**)             | 10단계 약관 동의를 두 개의 체크박스로 분리                       |
| 6   | 주차 상세      | `parkingType` enum(`freeAvailable`·`freeFirstCome`·`paid`) + optional 부가 설명  | 라벨을 문자열로 합치던 우회 제거, 선택지와 설명을 각각 전송      |
| 8   | 자동 임시저장  | `PATCH /host/rooms/drafts/{draftId}/autosave` + `DraftDetail.autosaves` 추가     | 응답 스키마만 반영. **저장 호출 연동은 아직 미구현** (별도 작업) |
